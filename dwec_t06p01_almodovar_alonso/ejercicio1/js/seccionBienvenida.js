const error = document.getElementById("errorBienvenida");
document.addEventListener('DOMContentLoaded', async () => {
    spinner.classList.remove("d-none");
    const datosAPI = await cargarAPI(); // [Gryffindor, Hufflepuff, Ravenclaw y Slytherin]
    let personajesAMostrar = [];
    if (datosAPI.length == 4) {
        for (let i = 0; i < datosAPI.length; i++) {
            const primero = datosAPI[i][getRandomInt(datosAPI[i].length)];
            let segundo = primero;
            while (primero == segundo) {
                segundo = datosAPI[i][getRandomInt(datosAPI[i].length)];
            }
